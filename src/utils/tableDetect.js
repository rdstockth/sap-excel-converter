...
export function hoursToText(r) {
  ...
  return lines(['[Hours] Work Order ' + v(r['MO no.']) + ' | Type: ' + v(r['MO type'] ?? r['MO Type']), fmt('Description', r['Description']), fmt('Notification', r['MN no.']), fmt('Equipment', r['Equipment']), fmt('Functional Location', r['Functional Location']), fmt('Inventory Number', r['Inventory number']), fmt('Cost Center', r['Cost Center']), fmt('Object Part', r['Object Part Text']), damage ? 'Damage: ' + damage : null, cause ? 'Cause: ' + cause : null, action ? 'Action: ' + action : null, fmt('MO Created Date', r['MO Created Date']), fmt('TECO Date', r['MO Teco Da']), fmt('Actual Labor Duration', r['Act. Labor duration']), fmt('Spare Part Cost (THB)', r['Spare Part Cost']), fmt('Consumed SP Cost (THB)', r['Consmpt SP Cost']), fmt('TS Service Cost (THB)', r['TS service Cost']), fmt('Total Cost (THB)', r['Total Cost'])])
}
